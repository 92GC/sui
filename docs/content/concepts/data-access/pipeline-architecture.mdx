...
<li><span className="text-sui-success-dark">✓</span> Sequential processing cannot keep up with your data volume.</li>
...
The `Broadcaster` and `Processor` components use identical backpressure mechanisms, `FANOUT` parallel processing, and `processor()` implementations to the concurrent pipeline. The `Processor` component is described in detail in the [Common pipeline components](#processor) section.
...
- The `Broadcaster` only fetches checkpoints up to the calculated boundary.
...
The `Collector` combines data from multiple checkpoints into single database write batches.
...
The `Pruner` operates in the range between the current `pruner_hi` and the safe boundary determined by `reader_lo`, ensuring readers are never affected. See the [watermark system](#watermark-system) for details on the three-watermark coordination.
...
#### Per-pipeline row-count backpressure

Each concurrent pipeline enforces a per-pipeline row limit via the `MAX_PENDING_ROWS` constant on its `Handler` trait. Before pulling the next checkpoint from the processor channel, the pipeline's collector waits until its own uncommitted row count drops below this threshold. Sequential pipelines do not use row-count backpressure because their in-order commit requirement can create deadlocks: the committer may need future checkpoints to make progress, but row-count backpressure prevents the collector from delivering them.

<ImportContent mode="code" source="crates/sui-indexer-alt-framework/src/pipeline/concurrent/mod.rs" variable="MAX_PENDING_ROWS" />

This is useful when pipelines produce highly variable numbers of rows per checkpoint. Channel-based backpressure counts checkpoints, not rows, so a single checkpoint with tens of thousands of rows can spike memory even though only one checkpoint is buffered. Because the limit is per-pipeline, pipelines that produce large rows (such as `kv_objects`) can be tuned independently from pipelines that produce small rows (such as `tx_digests`). When a pipeline's collector pauses, the processor-to-collector channel fills up, which blocks processor workers, which in turn fills the ingestion channel, creating backpressure on the broadcaster without the broadcaster needing to know about row counts.
...
1. **Backpressure cascade**:
    ...
    - `Broadcaster` stops fetching new checkpoints and sending to `Processors` (channel full).
1. **End result**:
...
- **`MAX_PENDING_ROWS`:** Controls how much data can accumulate before the collector pauses. Higher values provide more buffer space but use more memory during bottlenecks.
