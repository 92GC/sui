...
See [GraphQL and General-Purpose Indexer](/concepts/data-access/custom-indexers.mdx) for more information on the stack.
...
<table>
	<tbody>
		<tr>
			<td>
				![Indexer stack components](./images/indexer_stack.svg "A diagram showing the indexer stack components")

				The indexer stack uses the following components:
				- [Archival service](/concepts/data-access/archival-store): A service that provides access to historical data beyond what full nodes or indexer databases retain.
				- [Consistent store](/concepts/data-access/graphql-rpc#working-with-consistent-store): Combined indexer and RPC service for indexing live data on-chain and serving queries for recent checkpoints.
				- [Full node](/guides/operator/sui-full-node): Nodes that track and verify the network's state, and serve read requests from clients.
				- [GraphQL](/guides/developer/accessing-data/query-with-graphql): A query language for APIs.
				- [Managed snapshots](/guides/operator/snapshots): Point-in-time view of a database's store.
				- [Remote store](/concepts/data-access/indexer-data-integration#remote-store): A remote storage location for checkpoint data.
			</td>
			<td>
				<table>
					<tbody>
					<tr>
						<td>Indexer connection params</td>
						<td>
							1a. `--remote-store-url`<br/>
							1b. `--database-url`
						</td>
					</tr>
					<tr>
						<td>Consistent store connection params</td>
						<td>
							2a. `--remote-store-url`<br/>
							2b. One of `--azure`, `--gcs`, `--http`, `--s3`<br/>
							2c. `--database-path`
						</td>
					</tr>
					<tr>
						<td>GraphQL connection params</td>
						<td>
							3a. `--fullnode-rpc-url`<br/>
							3b. `--database-url`<br/>
							3c. `--consistent-store-url`<br/>
							3d. `--ledger-grpc-url`
						</td>
					</tr>
					</tbody>
...
## Add new pipeline to pre-existing indexer

Adding a new pipeline to an existing indexer currently requires these manual steps:

1. Determine the most recently indexed checkpoint (`recent-checkpoint`) by doing one of the following:
    * Query with GraphQL: `curl -d '{"query":"{ checkpoint { sequenceNumber } }"}' '<graphql-url>'`
    * Use the `indexer_alt_indexer_latest_processed_checkpoint` metric
2. Start the indexer with the new pipeline specifying
    * `--first-checkpoint <recent-checkpoint>`

   Note: `--first-checkpoint` only needs to be specified the first time the indexer runs with the new pipeline in order to bootstrap the watermark record; it will resume from the watermark record on subsequent runs.

   Caution: Other pipelines in the same indexer instance will stall until the new pipeline is caught up to the same checkpoint. Therefore, we recommend using a `recent-checkpoint` value as close to the network tip as possible.
3. If you also want to index checkpoints before `recent-checkpoint`, start a backfill on a separate indexer instance for the new pipeline specifying
    * `--first-checkpoint <min-checkpoint>`
    * `--last-checkpoint <recent-checkpoint - 1>`
