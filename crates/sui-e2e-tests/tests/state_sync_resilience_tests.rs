...
mod test {
    ...
    async fn test_disconnect_consistently_failing_peers() {
        ...
        let state_sync_config = StateSyncConfig {
            ...
            min_peers_for_disconnect: Some(1),
        ...
        // Very brief pause to let connections establish
        ...
        let initial_checkpoint = syncing_node.sui_node.with(|n| {
            n.state()
                .get_checkpoint_store()
                .get_highest_synced_checkpoint_seq_number()
                .unwrap()
        ...
        tracing::info!(
            "Syncing node starting at checkpoint {}, latency injected",
            initial_checkpoint
        );
